...
class Login_window:
    def __init__(self,root):
        ...
        img2 = Image.open("login_background.jpg")
        img2 = img2.resize((1600,700),Image.LANCZOS)
    ...
    def login(self):
        if self.txtuser.get()=="" or self.txtpass.get()=="":
            ...
            conn = mysql.connector.connect(host="localhost",user="root",password="r1@2m3N4",database="hotel_management")
            my_cursor = conn.cursor()
            my_cursor.execute("select * from register where email=%s and pass=%s",(self.txtuser.get(),self.txtpass.get()))
